...
func (t *Git) Update(path string, content string) error {
	...
	resp, err := t.request("PUT", t.getApi(path), encoding.JSONEncode(param))
	if err != nil {
		return err
	}
...
func (t *Git) Delete(path string) error {
	...
	resp, err := t.request("DELETE", t.getApi(path), encoding.JSONEncode(param))
	if err != nil {
		return err
	}
	_, err = t.getResp(resp)
...
func (t *Git) getSha(path string) string {
	resp, _ := t.Get(path)
	return json.New(resp).Key("sha").ToString()
}
...
func (t *Git) getResp(resp string) (string, error) {
	message := json.New(resp).Key("message").ToString()
	if message != "" {
...
func (t *Git) GetLastVersion() string {
	...
	return json.New(resp).Index(0).Key("name").ToString()
}
...
func (t *Git) GetContent(path string) string {
	...
	return encoding.Base64Decode(json.New(resp).Key("content").ToString())
}
